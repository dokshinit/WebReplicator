 ...
 * <p>
 * Типичное использование:<br> Создание экземпляра FB_Base (при этом происходит создание источника данных), который
 * умеет создавать соединения к базе FB_Connection и запросы к базе FB_Query. Запросы к базе данных выполняются в
 * помощью объектов FB_Query. Запрос может создаваться как независимый (в этом случае он сам открывает отдельное
 * соединение к базе при выполнении и закрывает его при закрытии запроса) или как выполняющийся в рамках существующего
 * соединения (в этом случае запрос не закрывает соединение при закрытии, а также не может делать commit и rollback -
 * для объединения запросов в одну транзакцию).
 *
...
public class FB_Database implements Closeable {
    ...
    /**
     * Сохранение данных в кеше после коммита (также подразумевает полный фетч данных в кеш при выполнении запросов!).
     * Значение по умолчанию для создаваемых соединений.
     */
     ...
     * @param url        Местоположение базы (jdbc:firebirdsql:хост[/порт]:путь к бд).
    ...
    /**
     * Закрывает пул соединений (чтобы процесс не висел - в вебапп приложение может не завершится).
     */
    @Override
    public void close() {
        if (dataSource != null) {
     ...
     *
     * @return true - если инициализация в конструкторе прошла успешно), false - если не прошла инициализация.
     */
